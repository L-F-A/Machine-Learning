...
#################################################################################################
        ...
        def train_Full(self):
			...
			try:
                        	L_k.append( np.linalg.cholesky(sig_temp))
			except np.linalg.linalg.LinAlgError:
				print 'Problem of positive definitness, using nearest matrix according to Frobenius norm'+' for class '+str(r)
				sig_temp,flag=nearestSPD(sig_temp,testPD=True,ModIfNotPD=True)
				L_k.append( np.linalg.cholesky(sig_temp))
	...
	def Query_Full(self,Xt,ClassOnly=False):
		if Xt.ndim==1:
				...
				#log of the unormalized probability
                        	logP[r]=np.log(self.pi_k[r]) -np.sum(np.log(np.diag(self.L_k[r]))) - 0.5*d2
                	delta=np.max(logP)
